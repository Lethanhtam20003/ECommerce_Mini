...
Xây dựng các Web API dịch vụ độc lập

Orchestration & Monitoring

.NET Aspire

Quản lý vòng đời container, Service Discovery, Telemetry Dashboard

API Gateway

YARP (Yet Another Reverse Proxy)

Định tuyến Request, Centralized Authentication & Rate Limiting

Database per Service

PostgreSQL

Cơ sở dữ liệu quan hệ lưu trữ dữ liệu Auth, Product, Order

In-Memory Data Store

Redis

Distributed Caching (Product), Cart State Management, API Rate Limiting

Message Broker (Nghiệp vụ)

RabbitMQ + MassTransit

Xử lý sự kiện bất đồng bộ chính xác (Order -> Notification -> Payment)

Event Streaming (Data Analytics)

Apache Kafka

Thu thập và xử lý luồng dữ liệu lớn (User Clickstream, Event Tracking)

Cloud Deployment

Azure Container Apps (ACA) / azd

Triển khai hạ tầng tự động lên đám mây thông qua Infrastructure as Code

3. Kiến trúc Chi tiết Các Sub-Systems (Services Breakdown)

ECommerce.AppHost: Project Aspire trung tâm định nghĩa hạ tầng, kết nối các container (Postgres, Redis, RabbitMQ, Kafka) và microservices.

ECommerce.ServiceDefaults: Cấu hình chuẩn cho OpenTelemetry (Tracing, Metrics, Logs), Health Checks, và Service Discovery.

ApiGateway (YARP): Điểm vào duy nhất cho Client. Tích hợp Redis Rate Limiting để chống DDOS và kiểm tra JWT Token.

AuthService: Quản lý tài khoản, phân quyền, phát hành JWT Access Token (Database: PostgreSQL).

ProductService: Quản lý Catalog sản phẩm & Tồn kho. Tích hợp Redis Distributed Cache cho các truy vấn Read-Heavy.

CartService: Quản lý Giỏ hàng mua sắm. Lưu trữ dữ liệu trực tiếp trong Redis Key-Value/Hash (không dùng RDBMS).

OrderService: Xử lý logic đặt hàng. Ghi đơn hàng vào Postgres và bắn event OrderCreatedEvent qua RabbitMQ.

NotificationService: Consumer lắng nghe OrderCreatedEvent từ RabbitMQ để phát email/tin nhắn xác nhận.

TrackingService: Thu thập log hành vi người dùng (xem sản phẩm, thêm giỏ hàng) qua Kafka Topic để phân tích Real-time.
...
[Giai đoạn 1] ──> [Giai đoạn 2] ──> [Giai đoạn 3] ──> [Giai đoạn 4] ──> [Giai đoạn 5] ──> [Giai đoạn 6]
  Aspire Core       Auth & Product       Redis Power        RabbitMQ Events      Kafka Stream       Cloud Deploy
...
Hiểu rõ vai trò từng công nghệ:
...
Khởi chạy Local 1-Click: Toàn bộ Microservices, Gateways và Databases khởi chạy thành công chỉ qua 1 nút bấm F5 hoặc dotnet run từ AppHost.

Observability hoàn chỉnh: Theo dõi được Distributed Tracing xuyên suốt từ ApiGateway -> OrderService -> RabbitMQ -> NotificationService trên Aspire Dashboard.

Sẵn sàng triển khai Cloud: Có khả năng đóng gói Docker và deploy thành công lên môi trường Cloud (ACA/AWS).